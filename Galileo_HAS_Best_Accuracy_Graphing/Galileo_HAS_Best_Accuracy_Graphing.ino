/*
  ...
  In this example we wait for HAS/E6 PPP convergence, then monitor the deviations (aka, accuracies).
  Once a certain additional amount of time has passed (120s), the device is reset.
  The goal is to see how good the positional fixes are using the Galileo/HAS E6 based corrections.
  ...
  Goals of this sketch:
    ...
    Reset once (PPP converged + timeInConvergenceBeforeReset)
    Avoid most prints
*/
...
unsigned long timeInConvergenceBeforeResetMs = 10 * 60 * 1000; //Reset after this many miliseconds in PPP convergence
...
void setup()
{
  ...
  delay(250);
  Serial.println();
  Serial.println("SparkFun UM980 Galileo HAS/E6 Accuracy Test");
...
void printUpdate()
{
  ...
  else if (positionType == 69)
  {
    ...
    {
      Serial.print("Seconds in PPP convergence: ");
      Serial.print((millis() - timeToConvergence) / 1000.0, 0);
      Serial.print("s");
    ...
    if (millis() - timeToConvergence > timeInConvergenceBeforeResetMs)
    {
      //Reset UM980
      ESP.restart();
    }
